...
if (!class_exists('Basilico_Page_Title')) {
    class Basilico_Page_Title
    {
        public function get_page_title(){
            ...
            $custom_class = basilico()->get_opt( 'page_title_class', '' );
            $pt_bg_parallax = basilico()->get_theme_opt('pt_bg_parallax');
            $data_parallax = $pll_cls = '';
            if( (isset($pt_bg_parallax) && $pt_bg_parallax) ){
                $data_parallax = json_encode([
                    basilico()->get_theme_opt('pt_parallax') => basilico()->get_theme_opt('pt_parallax_value')
                ]);
            ...
            if ($pt_mode == 'bd' && $ptitle_layout > 0 && class_exists('Pxltheme_Core') && is_callable( 'Elementor\Plugin::instance' )) {
                ?>
                <div id="pxl-pagetitle" style="<?php if(!empty($bg_image)){?>background-image:url('<?php echo esc_url($bg_image); ?>');<?php } ?>" class="pxl-pagetitle bg-image layout-el relative <?php echo !empty(esc_attr($custom_class)) ? $custom_class : ''; ?> <?php echo esc_attr($pll_cls) ?>">
                ...
                ?>
                <div id="pxl-pagetitle" style="<?php if(!empty($bg_image)){?>background-image:url('<?php echo esc_url($bg_image); ?>');<?php } ?>" class="pxl-pagetitle bg-image layout-df relative <?php echo esc_attr($custom_class) != '' ? $custom_class : ''; ?> <?php echo esc_attr($pll_cls) ?>">
                    <div class="pxl-page-title-bg pxl-absoluted" data-parallax='<?php echo esc_attr($data_parallax);?>'></div>
