<div class="pxl-dashboard-wrap">
	...
	elseif( !empty($plugin_requires) && sizeof($plugin_requires) >= 1 ):
		...
		echo sprintf( wp_kses_post( esc_html__( 'Make sure to activate required plugins prior to import a demo to %s. %sActive Now%s', 'basilico' ) ), basilico()->get_name(), '<a class="nt-atpli" href="' . admin_url( 'admin.php?page=pxlart-plugins') . '">' , '</a>' );
					...
					<div class="pxl-col pxl-col-12">
						<div class="pxl-imp-skip-posts">
							<span class="pxl-imp-opt-skip-posts" style="margin-bottom: 0; padding-left: 15px;">
								...
								<span><?php esc_html_e( 'Skip the posts existen, ( Default clear all content ).', 'basilico' ); ?></span>
								</label>
							</span>
						</div>
	...
	<div class="pxl-progress-popup pxl-imp-popup-wrap">
		<div class="pxl-imp-progress">
			...
			<div class="pxl-progress importing"><?php esc_html_e( 'Working', 'basilico' )?> <span>.</span><span>.</span><span>.</span></div>
			<div class="pxl-progressbar">
				<div class="pxl-progressbar-inner" style="width: 0%">
					<span class="pxl-loader pxl-progressbar-percentage"><?php esc_html_e( '0%', 'basilico' ); ?></span>
				</div>
			</div>
		</div>
	</div> 
