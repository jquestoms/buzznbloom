...
class Pxl_Elementor
{
    ...
    public function __construct()
    {
        ...
        if (is_file(get_stylesheet_directory() . '/elements/element-templates.php')) {
            require_once(get_stylesheet_directory() . '/elements/element-templates.php');
        }elseif (is_file(get_template_directory() . '/elements/element-templates.php')) {
    ...
    public function register_widgets($widgets_manager)
    {
        ...
        $folder_child = apply_filters( 'pxl-register-widgets-folder-child', get_stylesheet_directory() . '/elements/register/' );
        ...
        if (is_dir($folder_child)) {
            $file_childs = scandir($folder_child);
            foreach ($file_childs as $file) {
                $pos = strrpos($file, ".php");
                if ($pos !== false) {
                    require_once $folder_child . $file;
                }
            }
        }
        ...
        if( !empty($pxl_widgets)){
            ...
            foreach ($pxl_widgets as $widget) {
                $new_widget = new Pxl_Widget_Base([], $widget);
